...
def test_dropping_near_left_edge_snaps_to_left(qapp: QApplication) -> None:
    ...
    try:
        buddy._sim.set_anchor(float(left + _EDGE_DOCK_THRESHOLD - 5), 400.0)
        buddy._maybe_edge_dock()
        assert buddy._sim.anchor[0] == float(left)
    finally:
        buddy.close()
...
def test_dropping_near_top_edge_snaps_to_top(qapp: QApplication) -> None:
    ...
    try:
        buddy._sim.set_anchor(
            float(left + 300), float(top + _EDGE_DOCK_THRESHOLD - 5),
        )
        buddy._maybe_edge_dock()
        assert buddy._sim.anchor[1] == float(top)
    finally:
        buddy.close()
...
def test_dropping_far_from_edge_does_not_snap(qapp: QApplication) -> None:
    ...
    try:
        buddy._sim.set_anchor(mid_x, mid_y)
        buddy._maybe_edge_dock()
        assert buddy._sim.anchor == (mid_x, mid_y)
    finally:
        buddy.close()
