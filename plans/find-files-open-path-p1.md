...
- `allowed_dirs` default is `["~/Documents", "~/Desktop", "~/Downloads"]` as strings, expanded at use sites like `data_dir` (`tokenpal/app.py:144`); the git root of `Path.cwd()` is appended when one exists **and the configured list is non-empty**. **Amended at the p2 review (operator, 2026-09-04):** an explicitly empty `allowed_dirs` is opt-out and returns no roots, because the unconditional append meant emptying the list silently left the tools searching whatever repo the buddy launched from, and the "names no folder that exists" refusal could never fire.
