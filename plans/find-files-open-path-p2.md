...
- Every result, from either backend, passes through the same post-filter: `resolve_inside` (roots) → `(resolved, root)`, `is_hidden_or_protected(resolved, root)`, `path_is_sensitive(str(resolved.relative_to(root)))`, then stat, sort by mtime desc, cap.
