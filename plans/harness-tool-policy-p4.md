...
- `docs/claude/actions.md` — p1 already rewrote this bullet's `allow_unprompted` / `writes_durable_sink` sentences at `3bbc46d`; extend rather than replace. The author checklist for the declared policy, beside the existing `reads_desktop_content` contract.
