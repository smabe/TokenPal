...
- `tokenpal/brain/personality.py` — a nudge prompt builder beside `build_git_nudge_prompt` (`:1046-1067`), the closest precedent: it branches on `is_finetuned` and interpolates `_identity_block()`, `_mood_line()`, `_sample_examples()` and `_voice_reminder()`. It takes the reminder's label and asks for one short line in the buddy's voice. Match its length discipline.
- `tokenpal/brain/orchestrator.py` — spawn generation from `tick()`'s return value:
  - `self._nudge_tasks: dict[str, asyncio.Task]` keyed by reminder id, each discarded in an `add_done_callback`. **Keyed, not a set**, because the overlap guard needs it: a reminder whose generation is still in flight when it comes due again must not start a second one — skip and emit the canned label. The nearest precedent, `_conversation_summary_tasks` (`:962-966`), is a plain `set` with `add_done_callback(discard)`; copy the discard idiom, not the container.
  - **Hold the reference.** Three of the five existing `create_task` sites (`:561`, `:577`, `:594`) keep none, so "match the precedents" would reproduce the GC-mid-flight bug. The dict above is the rule.
  - `asyncio.wait_for(..., timeout=_NUDGE_TIMEOUT_S)`; on `TimeoutError` or any exception, emit the canned label. The in-repo pattern for a bounded await over off-loop work is `_new_conversation_session` (`:975-986`), which uses `shield` plus a `TimeoutError` fallback — read it before writing this.
  - Deliver through p4's `_emit_nudge(text, generated=True)` so the drift guard runs on generated text.
- `tests/test_brain/test_nudge_emission.py` — extend:
  6. a drifted generation (use a `_META_MARKERS` entry from `tokenpal/util/text_guards.py:12-25`, e.g. `"wikipedia"`) is rejected by `filter_response` and the canned label ships instead.
