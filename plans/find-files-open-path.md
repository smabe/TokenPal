...
## Phase map
**Phase p1 — shared path safety + `[paths] allowed_dirs`**
...
**Phase p2 — `find_files` action (Spotlight on macOS, bounded walk elsewhere)**
...
NEXT: p1 — read `plans/find-files-open-path-p1.md` FIRST. Binding decisions: containment is `resolve()` + `is_relative_to` against resolved roots, strict case on POSIX and `os.path.normcase` on Windows, returning `(resolved, root)`; `allowed_dirs` defaults to Documents/Desktop/Downloads plus the cwd git root and is read at execute time through `load_config()`; path sensitivity uses the narrow `contains_sensitive_content_term` plus `keychain`; the two git-root helpers collapse into one.
