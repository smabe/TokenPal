...
- `tokenpal/util/untrusted_text.py` — p1 — new: `scrub_body` and its placeholder
...
- **Sensitive-app filter for source apps is `contains_sensitive_term`** (full `SENSITIVE_APPS`, `tokenpal/brain/personality.py:285-290`), matching the kill switch (`check_sensitive_app`, `personality.py:769-771`). Body scrubbing is line-wise `scrub_body`.
...
- `pytest`, `ruff check tokenpal/`, `mypy tokenpal/ --ignore-missing-imports` green at the end of each phase.
...
- **`http_backend.py:448` logs raw tool-call arguments at WARNING on JSON failure.** Once a content tool exists, an argument could carry content. Revisit in #52 when the first marked tool lands.
