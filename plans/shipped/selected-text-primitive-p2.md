...
### Finding: live criteria closed by the operator 2026-09-04
- `[tools] enabled_tools = ["read_selection", "agent_mode"]` set; `tokenpal --check` listed `read_selection`; `/tools describe read_selection` reported `platforms: darwin`, `safe: True`, `cacheable: false`; `/agent tell me what I selected` read the TextEdit selection through the #51 unpersisted path. Operator report "tested all as working"; `chat_log` sweep afterwards found zero rows containing the selection text and zero hits in `~/.tokenpal/logs/`.
