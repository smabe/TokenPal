...
- **Phase 6 (automated verification)** — `ruff check tokenpal/` clean. `pytest` 1946 passed; 9 failures are all pre-existing and unrelated to size-me-up (Windows POSIX-mode chmod asserts × 5, filesystem_pulse × 3, modelfile path-separator × 1). `mypy tokenpal/` shows 12 pre-existing errors in 5 untouched files (`platform.py`, `console_overlay.py`, sense `_client.py` files); targeted mypy on size-me-up's touched modules is clean.

Open work:
- **Manual smoke** — run `./run.ps1`, exercise the bottom-right grip on the buddy. Confirm: grip is visible + grabbable, drag wholesale-scales buddy + bubble + dock + sky + rain in lockstep, layout looks correct at 0.5× / 1.0× / 2.0×, nothing falls offscreen, buddy physics still feels alive (swing, drag, edge-dock), zoom survives a restart. After that: `/plan ship size-me-up`.

Pre-existing user WIP still uncommitted (out of phase 4's scope):
- `tokenpal/ui/ascii_props.py` — sun/moon sprites tweaked with `▀`/`▄` rounded ends. Tangential to set_zoom; ship as a standalone commit or fold into phase 6.
