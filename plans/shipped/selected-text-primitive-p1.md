...
### Finding: live observations closed by the operator 2026-09-04 (Qt overlay, MTPLX)
- **Qt-host source-app rule confirmed.** With the Qt chat window focused and TextEdit behind it, `/proofread` produced `> proofread: 581 chars from TextEdit` — the same 581-char selection p1's terminal-host probe read. The frontmost normal window was the source app, as the repaired rule assumes.
