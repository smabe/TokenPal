...
p1 SHIPPED 521bdc0. p2 SHIPPED 1150dd9. All phases shipped 2026-09-02; feature stays opt-in after the dogfood table in `plans/agent-thinking-p2.md` showed no win on this Mac. Binding decisions for p2 (as shipped):
...
**Spec check at p2** — 8/8 Work items evidenced · none unclaimed. Screenshot criterion met in adapted form: the `…` line lands on the final text turn because MTPLX emits no reasoning on tool-call turns.
Parking-lot disposition at ship (2026-09-02): all five items kept as one follow-up to file as a GitHub issue on the operator's go (global rule: no external issues without a separate ask): train_voice thinking-controls duplicate + IndexError + config reloads; `_force_synthesis` bypassing `[agent]` caps; the two enumerated-keyword FakeLLMs; uncapped persisted reasoning; the `log_callback` type understating `markup=`/`url=`.
