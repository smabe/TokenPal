...
### Finding: live observations 2026-09-04 (operator at the keyboard, Qt overlay, MTPLX Qwen3.8-27B)
- **Check 1 (consent):** `/consent` granted `desktop_content` ("5 granted").
- **Check 2 (proofread from TextEdit): PASSED.** Chat pane showed `> proofread: 581 chars from TextEdit`, the paragraph verbatim with `Changes:` / `No changes needed.`, then the persona bubble ("Woah! Hold up, man! The answer is right there in the chat log! …"). `sqlite3 ~/.tokenpal/memory.db`: the last six `chat_log` rows are the bubble, the status line, `/proofread`, and the consent lines; zero rows contain the paragraph's text; zero `conversation_summaries` rows; zero hits in `~/.tokenpal/logs/`. The reply was not cut off (no reply-limit marker) with thinking configured off on MTPLX.
- **Checks 3-7: PASSED** (operator 2026-09-04, "tested all as working"). `chat_log` afterwards shows the sequence: the sensitive refusal ("Not now — sensitive window is open." — with Messages frontmost the brain's kill switch fired before the app-name gate, which is also a refusal with no read), `/consent` revoke → "Tool requires 'desktop content' consent. Open /consent to grant it.", `/proofread blah blah` → `> proofread: 9 chars typed` + persona bubble, re-grant, and the nothing-selected read `> proofread: 8000 chars from TextEdit (whole field — nothing was selected) (truncated)`. Zero `chat_log` rows contain any read or typed text; zero log-file hits. The operator noted the 8,000-char whole-field cap and called it fine.
