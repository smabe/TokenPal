...
## Parking lot
Dispositioned at ship, 2026-09-02. All six items filed; none dropped.
- **Stale function-local `load_config` import** at `train_voice.py:452`, plus the `_config_or_defaults` duplication between `_ollama_generate` and `_get_voices_dir` -> filed as #46 (bug, ready-for-agent). Both were excluded by the scope contract and share one file and one fix.
- **Trainer inherits `per_server_models`**, model auto-adopt, and the local-Ollama fallback, all invisible to the user -> filed as #47 (bug, ready-for-agent).
- **Unreachable-remote cost is still 2x the old ceiling** (240 s vs 120 s) -> filed as #48 (enhancement, ready-for-agent), carrying the two rejected probe-shortening candidates and the `app.py:566` interaction so they are not re-derived.
- **`_model_available` written in 5 places, read nowhere** -> filed as #49 (bug, ready-for-agent). Pre-existing; linked from #48 because it is the same fail-fast gap.
- **`ScriptedLLM` recorder asymmetry** -> filed as #50 (good first issue, ready-for-agent).
