...
- **Phase 4** — wire `set_zoom(factor)` on `BuddyWindow`, `SpeechBubble`, `ChatDock`. See "Files to touch" entries below for per-file detail.
- **Phase 5** — `QtOverlay.set_zoom` orchestrator. Clamp `[0.5, 2.5]`, fan out, persist to `UiState`. Wire `BuddyResizeGrip.zoom_drag_delta` into the orchestrator. Restore zoom on startup. **Latent bug to address here**: today `app.py:_persist_ui_state` callback signature is `(buddy_visible, windows)` and would silently clobber `zoom=1.0` on every visibility toggle. Either extend the callback signature, or have `app.py` read existing zoom from disk before writing.
- **Phase 6** — full `pytest` / `ruff check tokenpal/` / `mypy tokenpal/` green, manual smoke at 0.5×, 1.0×, 2.0×.

Pre-existing user WIP still uncommitted at session start, untouched by this work:
- `tokenpal/ui/ascii_props.py` — sun/moon sprites tweaked with `▀`/`▄` rounded ends. Already aligned with the parking-lot follow-up; pick up in phase 4 or 6 if desired.
- `tokenpal/ui/qt/buddy_window.py` — adds `paint_block_char` import + uses it in `paintEvent` block-fill. Will get touched naturally in phase 4.
