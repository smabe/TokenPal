...
A tool that takes a filesystem path **declares** the policy and writes no containment of its own. `ToolInvoker.invoke` is the single dispatch point; before the rate-limit block it screens, resolves and contains every declared argument, and substitutes a `ResolvedPath` into a copy of the arguments. `tests/test_actions/test_path_policy_contract.py` fails any registered tool whose JSON Schema declares a property whose name contains `path`, `file`, `dir` or `folder` and that does not name it in `path_params` — the selector is the schema, so forgetting the declaration is what it catches.
...
- `path_params` empty is also the performance gate: the invoker does no path work for a tool that declares none, which keeps a `git rev-parse` fork off every non-path tool call. Both other ClassVars are inert without it.
- `path_roots = "git_root"` admits the cwd's git worktree and refuses outside one; a relative argument anchors at the repo root. `"allowed_dirs"` admits the folders in `[paths] allowed_dirs` (plus the cwd git root, per above), and a relative argument stays anchored at the process cwd because N roots have no single anchor.
- `path_screen` governs the screen on the **raw** argument (the resolved name is always screened with `path_is_sensitive`), and the same strength drives the output-side `is_screened_out` for tools that filter paths a backend returned. `"broad"` applies `REJECT_PATH` and `contains_sensitive_term`; `"narrow"` applies neither, because on an absolute path both refuse a benign file under a badly-named folder (`<root>/credentials-app/README.md`). The **resolved** name is always screened with `path_is_sensitive`, whichever you declare. Raw-then-resolved is two screens on purpose: the raw one alone lets a symlink launder a denied name, the resolved one alone drops the broad app terms.
- Refusals are fixed strings keyed on the policy and the screen, and never echo the argument — the filename itself can be the sensitive part.

### What the tool must still write

- **Refuse a non-`ResolvedPath`.** The invoker skips containment when a declared argument is absent, blank, or not a `str`, and hands `execute` the raw value. Start with `if not isinstance(path, ResolvedPath): return <refusal>` — falling through to a default target is how `grep_codebase` used to turn a JSON number into a whole-repo search. The contract test asserts this mechanically for every declaring tool.
- **Use `path.resolved`** for anything that touches the filesystem, including a subprocess argv: validating the argument while passing the raw spelling leaves the OS walking the unvalidated target. `path.raw` exists only for a tool that must compare spellings (`read_file`'s dual `git ls-files` check), `path.root` for `git -C` and the resolved-target checks `open_path` keeps, `path.rel` as a posix string for a git pathspec.
- **Screen the output too, when the paths come from a backend rather than from the caller.** Containment screens the target's own name; neither Spotlight nor ripgrep honours a root's hidden or ignored files once pointed at one explicitly, so `find_files` filters its candidates and `grep_codebase` its hits with `is_screened_out(resolved, root, rel, screen)` (`tokenpal/util/paths.py`). Without it, `grep_codebase(path=".git")` returned `.git/description`'s contents.
