...
- Keyboard shortcuts: F1=/help, F2=toggle chat log, F3=/options, F4=/voice, Ctrl+L=/clear, Ctrl+C=quit
...
- `/options` — umbrella settings modal (also on F3). One section today (**Chat history**: digit-sanitized `max_persisted` input + "Clear history now" button) plus launcher buttons that dismiss-then-open the existing Cloud / Senses / Tools modals (`_open_cloud_modal` / `_open_senses_modal` / `_open_tools_modal` in `app.py`). Modal is in `tokenpal/ui/options_modal.py`; result dataclass carries `navigate_to: Literal[None, "cloud", "senses", "tools"]`. `/clear` (Ctrl+L) also wipes persisted rows — the modal's "Clear history now" is the discoverable version
