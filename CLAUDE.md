...
- `typing_cadence` — WPM buckets (`idle`/`slow`/`normal`/`rapid`/`furious`) from a rolling 15s keypress window. Emits on bucket transitions, at the 10-minute mark of a sustained `rapid`/`furious` run, and on post-burst silence. Subscribes to `tokenpal/senses/_keyboard_bus.py` — a shared singleton pynput listener that also feeds `idle`. The sense itself never imports pynput and never sees key values; test suite enforces this with a source-grep assertion. Opt-in via `[senses] typing_cadence = true`
