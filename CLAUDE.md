...
- Idle tool rolls (M1 + M2 shipped 2026-04-17): third emission path — `IdleToolRoller` in `tokenpal/brain/idle_tools.py`, rules in `idle_rules.py`. Fires only when the comment gate chose silence, so cannot inflate comment rate. 11 rules: M1's 9 (evening_moon, morning_word, monday_joke, weather_change, long_focus_fact, deep_lull_trivia, on_this_day_opener, lunar_override, memory_recall) plus M2's `todays_joke_bit` (callback-only running bit, 4h decay) and `morning_monologue` (chains weather_forecast_week + sunrise_sunset + on_this_day into one riff). `morning_word` upgraded to running-bit (8h decay) — the word rides along every subsequent prompt as a soft "slip it in once naturally, never re-define" instruction until decay. Evergreen daily tools (word/joke/on_this_day/moon_phase/sunrise_sunset) warmed once at session start and cached 6h so the hot path never blocks on HTTP. Running bits live on `PersonalityEngine.add_running_bit()` — max 3 active with LRU eviction, same-tag replace. Per-rule cooldowns + `[idle_tools] global_cooldown_s` + `max_per_hour` rate cap. Every fire writes `idle_tool_fire` to memory.db for future memory_query callbacks. `callback_book` cut from M2 (needs a curated author list we couldn't justify without LLM classification). M3 LLM-initiated tool-calling deferred to issue #33. See `plans/idle-tool-rolls.md`
