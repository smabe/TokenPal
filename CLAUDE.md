...
- `find_files` (opt-in, `[tools] enabled_tools`) searches `[paths] allowed_dirs` — an explicitly empty list disables it, and the cwd git root is added only when the list is non-empty. It returns paths and mtimes, never file bytes, and deliberately carries **no** `reads_desktop_content` marker. **Accepted exception (operator, 2026-09-04):** on macOS its Spotlight predicate includes `kMDItemTextContent`, so a query can match a file by its *contents* and return that path — the LLM can therefore test whether a string exists under an allowed root. Unlike the marked desktop-content tier, those paths persist (agent trace, chat log, `conversation_summaries`). Filenames are filtered by `path_is_sensitive` (`tokenpal/util/paths.py`), which is narrower than `contains_sensitive_term`: bank and messaging words stay allowed by choice, key material and password managers do not. So the desktop-content tier below is not the only path by which content-derived information reaches the model
