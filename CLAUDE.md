...
- `/cloud [status|enable <key>|disable|forget|model <id>]` — opt-in Anthropic-backed synth for `/research` only. Key stored at `~/.tokenpal/.secrets.json` (0o600), never in `config.toml`. Fingerprint-only in status output; raw key scrubbed from result. Fallback to local synth on any `CloudBackendError` (auth, `no_credit`, rate_limit, timeout, network). See `plans/shipped/claude-api-research-synth.md`.
