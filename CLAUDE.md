...
- `/cloud [status|enable <key>|disable|forget|model <id>|plan on|off|search on|off|deep on|off]` — opt-in Anthropic-backed synth for `/research` only. Key stored at `~/.tokenpal/.secrets.json` (0o600), never in `config.toml`. Fingerprint-only in status output; raw key scrubbed from result. Fallback to local synth on any `CloudBackendError`. Three cloud-web modes (deep wins over search when both are on): **`/cloud search on`** (Sonnet+ only) attaches `web_search_20260209` only — Sonnet drives the search loop but never fetches full pages, so input tokens stay bounded (~$0.15-0.25/run). Good middle tier. **`/cloud deep on`** (Sonnet+ only) adds `web_fetch_20260209` — each fetch loads full page content and the tool-loop re-bills it on every step, so real cost is $1-3/run on review-heavy queries. The slash handler emits a cost warning on activation. Deep/search sources have no cached excerpts, so `/refine` refuses after a deep/search run. See `plans/shipped/claude-api-research-synth.md` and the "Deep mode" subsection of `docs/research-architecture.md`.
