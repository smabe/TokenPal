...
- `/research <question>` — plan → search → fetch → synth → validate pipeline with JSON-structured synth output, substring-grounded claim validation, two-stage fetch (newspaper4k primary, aiohttp fallback). Multi-backend search layer: DDG (free default), Tavily (preloaded content, `/cloud tavily`), Brave (free tier, `/cloud brave`), HN (keyless), StackExchange (keyless). The planner picks a backend per query via the `backend` field in its JSON output; `_resolve_backend` normalizes + falls back safely on hallucinated or unconfigured backends. End-of-run `telemetry: mode=<backend>=<N>,... sources=<N> stopped=<reason>` lands in the session log. Full architecture in `docs/research-architecture.md` — read before editing any of `tokenpal/brain/research.py`, `tokenpal/actions/research/fetch_url.py`, or the conversation system prompt in `tokenpal/brain/personality.py`.
- `/refine <follow-up>` — re-analyze the most recent `/research` run's cached sources against a new question. Cloud-only (`/cloud enable` required). Refuses after `/cloud deep|search` runs because those have no cached excerpts. Source cache lookup via `MemoryStore` with max-age gate. Wiring in `tokenpal/brain/orchestrator.py` + `tokenpal/app.py:676`.
