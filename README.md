...
| **Actions** | Timers, system info, open apps, safe math eval — plus opt-in local (git/grep/read_file), utility (currency, weather forecast, CoinGecko, etc.), and focus (pomodoro, recurring reminders) tools. All gated through a Textual picker with per-category consent. Optional per-tool rate limits + usage stats in `memory.db` |
