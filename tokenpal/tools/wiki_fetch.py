...
def _wikitext_to_dialogue(wikitext: str) -> str:
    ...
    for raw_line in wikitext.splitlines():
        ...
        if m:
            ...
            # Clean wiki formatting from dialogue
            dialogue = re.sub(r"'''?", "", dialogue)  # bold/italic markers
            dialogue = re.sub(r"''", "", dialogue)
            dialogue = re.sub(r"\[\[([^\]|]*\|)?([^\]]+)\]\]", r"\2", dialogue)  # [[links]]
            dialogue = re.sub(r"\{\{[^}]*\}\}", "", dialogue)  # {{templates}}
